...
namespace Lab6
{
    internal class Program
    {
        ...
        static void serilizeFirstItem() 
        {
            
            // Serilize the Object Index 0 for first element 
             var file = JsonSerializer.Serialize(elements[0]);
            //Creates anew file and writes element[0](index) 
            File.WriteAllText("imthefile.json", file);
        }

        static void serilaizeWholeCollection()
        {
            // SerlizeElement 
            var file2 = JsonSerializer.Serialize(elements);
            // Creates a file with all elements 
            File.WriteAllText("iHaveAllCollections.json", file2);
        }
